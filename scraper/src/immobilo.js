...
const scrapeData = async ({ page, collection, type, logEvent, searchUrl }) => {
    try {
        ...
        const prevEntries = await collection.find({ provider: "immobilo.de", searchUrl }, { projection: { url: 1 } }).toArray();
    
        while (lastPage && currentPage <= lastPage && !error) {
            console.log('Immobilo SCRAPING', currentPage, 'OF', lastPage, searchUrl);
            ...
            const [links, newLastPage] = await page.evaluate(() => {
                ...
                linkElements.forEach(el => {
                    ...
                    if (url) {
                        urls.push(`https://www.immobilo.de${url}`);
                    }
                });
        ...
        if (type === 'FULL_SCAN' && !error) {
            ...
            const message = `Immobilo - Scraped ${count} new estates and removed ${result.deletedCount} old estates.`;
            await logEvent({ scraper: 'immobilo.de', success: true, message });
            console.log(message);
        } else if (!error) {
            const message = `Immobilo - Scraped ${count} new estates`;
            await logEvent({ scraper: 'immobilo.de', success: true, message });
            console.log(message);
        } else if (error) {
            await logEvent({ scraper: 'immobilo.de', success: false, message: `Errors occured on scraping Immobilo, it still scraped ${count} new estates` });
        }
    } catch (error) {
        const message = 'Immobilo - Unexpected occured ' + error;
        console.error(message);
        await logEvent({ scraper: 'immobilo.de', success: false, message });
    }
}

const scrapeUrls = [
    'https://www.immobilo.de/suchergebnisse?l=Germany&r=0km&_multiselect_r=0km&t=all%3Arental%3Aliving&a=de.deutschland&pf=0&pt=10&rf=&rt=&sf=&st=&psf=&pst=&yf=&yt=&ff=&ft=&ba=&ro=0&s=most_recently_updated_first&pa=&o=&ad=&u=',
    // 'https://www.immobilo.de/suchergebnisse?l=Germany&r=0km&_multiselect_r=0km&t=all%3Arental%3Aliving&a=de.deutschland&pf=11&pt=25&rf=&rt=&sf=&st=&psf=&pst=&yf=&yt=&ff=&ft=&ba=&ro=0&s=most_recently_updated_first&pa=&o=&ad=&u=',
    // 'https://www.immobilo.de/suchergebnisse?l=Germany&r=0km&_multiselect_r=0km&t=all%3Arental%3Aliving&a=de.deutschland&pf=26&pt=35&rf=&rt=&sf=&st=&psf=&pst=&yf=&yt=&ff=&ft=&ba=&ro=0&s=most_recently_updated_first&pa=&o=&ad=&u=',
...
export const immobiloCrawler = crawler;
