...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const newLead = await fetch(`${process.env.NOCODB_URI}/api/v2/tables/ml7dpwwbdlxn92i/records`, options)
        .then(res => res.json())

      const url = `${process.env.NOCODB_URI}/api/v2/tables/${tableIds[type]}/records`
      ...
      await fetch(`${process.env.NOCODB_URI}/api/v2/tables/ml7dpwwbdlxn92i/links/${linkId}/records/${newLead.Id}`, linkOptions)
