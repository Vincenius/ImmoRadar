...
export default async function handler(req, res) {
  if (req.body.api_key !== process.env.API_KEY) {
    ...
    const fileIds = await Promise.all(files.map(async file => {
      ...
      const newFile = await fetch('https://immoradar.xyz/api/v2/tables/mp3txseo70f4win/records', fileOptions).then(res => res.json())

      return { Id: newFile.Id }
    }))

    const linkOptions = getOptions(fileIds)
    await fetch(`https://immoradar.xyz/api/v2/tables/${tableId}/links/${linkId}/records/${recordId}`, linkOptions)
      .then(res => res.json())
