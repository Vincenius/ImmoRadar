export default async function handler(req, res) {
    ...
    try {
        const url = `https://immoradar.xyz/api/v2/tables/ml7dpwwbdlxn92i/records?where=(Token,eq,${token})`;
        const options = {
            method: 'GET',
            headers: {
        ...
        if (list[0] && list[0].Id) {
            const url = `https://immoradar.xyz/api/v2/tables/ml7dpwwbdlxn92i/records`;
            const options = {
                method: 'PATCH',
                headers: {
                    'xc-token': process.env.NOCODB_KEY,
