...
const libIsoCountries = countries.getNames('de', { select: 'official' });
...
const countryOptionsDataMap = Object.fromEntries(
  libPhoneNumberCountries
    .map((code) => {
      ...
      const number = getCountryCallingCode(code)
      if (!name || !emoji || !number) return null;
      return [
        code,
        {
          code,
  ...
  const options = countryOptionsData
    ...
    .map((item) => (
      <Combobox.Option
        ...
        key={item.code}
      >
        <Group gap="xs" bg={item.code === country.code ? 'gray.3' : 'transparent'}>
          <span>
            {item.emoji} (+{item.number}) {item.name}
          </span>
        </Group>
      </Combobox.Option>
    ));
    ...
    >
      <Combobox.Target withAriaAttributes={false}>
        <ActionIcon
          ...
          w={leftSectionWidth}
        >
          <Text c="gray" mr="xs">{country.emoji} +{country.number}</Text>
        ...
        <Combobox.Options>
          <ScrollArea.Autosize mah={200} type="scroll">
            {options.length > 0 ? (
              ...
              <Combobox.Empty>Nicht gefunden</Combobox.Empty>
            )}
          </ScrollArea.Autosize>
        </Combobox.Options>
      </Combobox.Dropdown>
